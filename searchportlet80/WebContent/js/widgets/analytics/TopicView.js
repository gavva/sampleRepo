...
// @endCopyright
//*********************** ES COPYRIGHT END  ***********************************
dojo.provide("widgets.analytics.TopicView");
...
widgets.analytics._AnalyticsPane,
{
	...
	templatePath: dojo.moduleUrl("widgets.analytics", "templates/TopicView.html"),
	...
	load: function(facetId, hasSubfacets) {
		if (!EDR.isNoflash) {
			var successCallback = dojo.hitch(this, function() {
				this.clear();
				TopicView.setApplicationProperties("showLineChart", this.showLineInChart ? "true" : "false");
				TopicView.setApplicationProperties("barColorType", this.barColorOfMultiChart);
				TopicView.load(facetId, hasSubfacets, this.verticalRowCount, this.url, "");
			});
			if (TopicView.initialized) {
				successCallback();
			} else {
				TopicView.lazyLoading = successCallback;
			}
		} else {
			var params = {
	...
	clear: function() {
		if (!EDR.isNoflash) {
			TopicView.addFlexEventListener("onSubmitSearch", dojo.hitch(this, "onSubmitSearch"));
			TopicView.addFlexEventListener("onLoadError", dojo.hitch(this, "loadFlexErrorHandler"));
		}
	},
...
//		dijit.byId("horizontalSearchPanes").resultToolbar.enableDeepInspectionButton(false);
	...
	onRowHeightChanged: function(evt) {
		...
		if (this.rowHeightSelection.isValid()) TopicView.rowHeight(rowHeight);
	},
